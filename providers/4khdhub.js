...
async function validateUrl(url) {
    try {
        ...
        const trustedHosts = ['pixeldrain.dev'];
        ...
        console.log(`[4KHDHub] URL validation for ${url}: ${res.status} - ${isValid ? 'VALID' : 'INVALID'}`);
        return isValid;
    } catch (error) {
        console.log(`[4KHDHub] URL validation error for ${url}: ${error.message}`);
        return false;
    }
}
...
async function makeRequest(url, options = {}) {
    try {
        const res = await axios({
            ...
            timeout: 30000,
            maxRedirects: options.allowRedirects === false ? 0 : 5,
        ...
        let $ = null;
        if (options.parseHTML && res.data) {
            $ = cheerio.load(res.data);
        }
        return { $: $, body: res.data, statusCode: res.status, headers: res.headers };
    } catch (err) {
        if (options.allowRedirects === false && err.response && [301, 302, 303, 307, 308].includes(err.response.status)) {
            return { statusCode: err.response.status, headers: err.response.headers };
        }
        throw err;
    }
}
