...
async function runProviderFunctionalChecks(){
	...
	for(const row of rows){
		...
		try {
			let checkUrl = `/api/streams/${name}/movie/${targetTmdb}`;
			if (name === 'anikoto') {
				// Use an anime series for AniKoto (e.g. One Piece S1 E1)
				checkUrl = `/api/streams/${name}/series/37854?season=1&episode=1`;
			}
