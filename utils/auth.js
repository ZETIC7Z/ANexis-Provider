...
function ensureAuthFile() {
  ...
  const defaultPassword = '42564126';
  const { hash } = hashPassword(defaultPassword);
  const data = {
    info: 'Initial default credentials. Username is permanently admin.',
    users: [ { username: 'admin', hash } ]
  };
  fs.writeFileSync(AUTH_FILE, JSON.stringify(data, null, 2));
  console.warn('[auth] Created default auth-users.json with username "admin" and password "42564126".');
}
